;;----------------------------------------------------------------------------;;
;; MORU NONMEM Pyronaridine -- 3 compartment model + 4 transit (F1)
;; 
;;----------------------------------------------------------------------------;;
;; Kuangyi Zhang
;; Date: 2023-05-11
...
$PROBLEM Pyronaridine -- 3 compartment model + 4 transit (F1)
$INPUT 		ID 
			DATE = DROP 						
			TIME 						
...
$PK			
			...
			V3 = THETA(5) * EXP(ETA(5)) 		; Individual volume for peripheral compartment 1
			Q1 = THETA(6) * EXP(ETA(6))	
			V4 = THETA(7) * EXP(ETA(7)) 		; Individual volume for peripheral compartment 2
			Q2 = THETA(8) * EXP(ETA(8))
...
$TABLE ID TIME TAD EVID NDDV DV MDV CMT ADMIN PQFLAG SEX HT WT IPRED IWRES NOPRINT ONEHEADER FILE=mytab54