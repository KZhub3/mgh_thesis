$PROBLEM Pyronaridine -- 3 compartment model + 4 transit (+WT with IIV)
$INPUT 		ID 
			DATE = DROP 						
			TIME 						
...
$PK			
			...
			V4 = THETA(7) * (WT/58.5)**1 * EXP(ETA(7)) 			; Individual volume for peripheral compartment 2
...
$TABLE ID TIME TAD EVID NDDV DV MDV CMT ADMIN PQFLAG SEX HT WT IPRED CWRES IWRES NOPRINT ONEHEADER FILE=mytab62

$TABLE ID IWRES TIME IPRED PRED EVID MDV CWRES NOPRINT ONEHEADER FILE=sdtab62
$TABLE ID CL V2 V3 MTT Q1 V4 Q2 F1 ETA1 ETA2 ETA3 ETA4 ETA5 ETA6 ETA7 ETA8 NOPRINT ONEHEADER FILE=patab62
$TABLE ID WT HT AGE WBC HB HCT PLT NEUT LYMPH MONO EO BASO METHM PROTN GLUC BUN CREATE DIRB TOTB ALBM ALP AST ALT NA K CK NOPRINT ONEHEADER FILE=cotab62
$TABLE ID SEX NOPRINT ONEHEADER FILE=catab62
